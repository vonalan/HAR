...
def numCenterAssign(numCenter, trainY):
    ...
    cn = numpy.mat(numpy.ones(numpy.shape(ct)))
...
def innerCluster(trainX, trainY, numCenter, alpha):
    ...
    for i in range(numpy.shape(cn)[1]):
        ...
        centroidsBest = kMeans(trainX[numpy.nonzero(trainY[:, i])[0]], int(cn[0, i]))
        U[begin:end, :] = centroidsBest
        '''bug bug bug bug'''
        '''the definition of radius of RBFNN'''
        '''bug bug bug bug'''
        '''compute V'''
        distM = cDist(trainX[numpy.nonzero(trainY[:, i])[0]], U[begin:end, :])
...
def doSect(lowerLimit, upperLimit, k):
    limitRange = upperLimit - lowerLimit
    middle = (lowerLimit + upperLimit)/2
    if limitRange < 1:
        print "Limit Error: limitRange. "
        return []
    elif limitRange == 1:
        return []
    else:
        if k < 1:
            ...
            return doSect(lowerLimit, middle, k - 1) + doSect(middle, upperLimit, k - 1)
